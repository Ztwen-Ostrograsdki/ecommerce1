    ...
    <nav class="max-w-[85rem] w-full mx-auto px-4 md:px-6 lg:px-8" aria-label="Global">
      <div class="relative md:flex md:items-center md:justify-between">
        <div class="flex items-center justify-between">
          <a class="flex-none text-xl font-semibold dark:text-white dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="/" aria-label="Brand">ZtweN eCom</a>
          <div class="md:hidden">
        ...
        <div id="navbar-collapse-with-animation" class="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow md:block">
          <div class="overflow-hidden overflow-y-auto max-h-[75vh] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-slate-700 dark:[&::-webkit-scrollbar-thumb]:bg-slate-500">
            <div class="flex flex-col gap-x-0 mt-5 divide-y divide-dashed divide-gray-200 md:flex-row md:items-center md:justify-end md:gap-x-7 md:mt-0 md:ps-7 md:divide-y-0 md:divide-solid dark:divide-gray-700">
              ...
              <a class="font-medium {{request()->is('/') ? 'text-blue-600' : 'text-gray-500' }} py-3 md:py-6  dark:focus:outline-none dark:focus:ring-1 hover:bg-gray-700 " href="{{route('home')}}" aria-current="page">Acceuil</a>
  
              <a class="font-medium {{request()->is('categories') ? 'text-blue-600' : 'text-gray-500' }} hover:text-gray-400 py-3 md:py-6  hover:bg-gray-700 dark:focus:outline-none dark:focus:ring-1 " href="{{route('categories.home')}}">
                Catégories
              </a>
              <a class="font-medium {{request()->is('articles') ? 'text-blue-600' : 'text-gray-500' }} hover:text-gray-400 py-3 md:py-6  hover:bg-gray-700 dark:focus:outline-none dark:focus:ring-1 " href="{{route('products.home')}}">
                Articles
              </a>
  
              <a class="font-medium flex items-center {{request()->is('mon-panier') ? 'text-blue-600' : 'text-gray-500' }} hover:text-gray-400 py-3 md:py-6  hover:bg-gray-700 dark:focus:outline-none dark:focus:ring-1 " href="{{route('my_cart')}}">
                ...
                <span class="mr-1">Panier</span> <span class="py-0.5 px-1.5 rounded-full text-xs font-medium bg-blue-50 border border-blue-200 text-blue-600"> {{ $total_items }} </span>
              </a>
  
              @guest
              <div class="pt-3 md:pt-0">
                <a class="py-2.5 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 " href="{{route('login')}}">
              ...
              <div class="hs-dropdown-menu transition-[opacity,margin] duration-[0.1ms] md:duration-[150ms] hs-dropdown-open:opacity-100 opacity-0 md:w-48 hidden z-10 bg-white md:shadow-md rounded-lg p-2 dark:bg-gray-800 md:dark:border dark:border-gray-700 dark:divide-gray-700 before:absolute top-full md:border before:-top-5 before:start-0 before:w-full before:h-5">
                ...
                <hr class="mb-2">
                @endif
                <a href="{{url('/administration')}}" class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1 " >
                  Administration
                </a>
                ...
                @if($total_items > 0)
                <a wire:click='clearCart' href="#" class="flex items-center cursor-pointer gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1 ">
                  Vider mon panier <b> ({{$total_items}}) </b>
                </a>
                @endif
                <a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1 " href="#">
                  Mon profil
                </a>
                <a wire:click='logout' href="#" class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1">
                  Deconnexion
                </a>
              </div>
              </div>
